...
1. **Gate** — save only if it is verified-success AND (recurrence OR high cost saved) AND
   generalizable. If any fails, skip the save (a failed attempt → a `gotcha` at `confidence: 0`).
...
4. **Scrub** (hard gate) — write the candidate to a temp file, then:
   ...
   Exit `0` = use the scrubbed text. **Exit `2` = REFUSE - fix and re-scrub before writing.**
5. **Dedup / supersede** — read the target store first. ADD / UPDATE / NOOP. On a
   ...
   A verified memory takes precedence over an unverified candidate.
...
- Store the METHOD, not the data - leave literal secrets out. `km-scrub.js` exit `2` = refuse.
