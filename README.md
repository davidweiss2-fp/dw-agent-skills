...
| `dw-env-skill` | "clone" / "set up the env" / "bootstrap" / "new machine" / `/dw-env [preflight\|bootstrap]` | Owns the canonical flat `{workspace}/{repo}` layout (never a namespace subdir) with a deterministic preflight - Docker daemon, AWS credentials shape, layout - that fails fast with remediation; a bootstrap flow for new machines and repos |
| `dw-post-merge-verification-skill` | "verify the fix landed" / "did it work in prod" / `/dw-post-merge [PR URL]` | Proves a merged PR changed prod behavior instead of trusting green CI - verifies locally, queries the plan-time success metric through read-only APM/analytics tools, hands the dev a checklist for the rest, then rules confirmed / no-effect / inconclusive; never touches prod beyond read-only queries |
